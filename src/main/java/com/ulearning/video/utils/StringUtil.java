package com.ulearning.video.utils;
...
public class StringUtil {
    ...
    private static final String urlStr = "https://apis.map.qq.com/ws/location/v1/ip";
    private static final String key = "VIBBZ-B7S34-4J3US-DGIS6-QA525-PUBD4";
...
// //		str = str.replaceAll("\"", "&quot;");
     ...
     * @param folderName
     * @return Date:2015-2-11上午10:57:20
     * @author WangHuayong
     * @since JDK 1.6
     */
    ...
    public static boolean checkGradeStr(String s) {
        boolean result = false;
        String pattern = "\\d{4}";
        result = Pattern.matches(pattern, s);
        return result;
    }
    ...
    /**
     * 解析ip地址   设置访问地址为http://ip.taobao.com/service/getIpInfo.php
     * 设置请求参数为ip=[已经获得的ip地址] 设置解码方式为UTF-8  
    ...
    public static String getAddresses(String content, String encoding) throws UnsupportedEncodingException {
        ...
        try {
            url = new URL(urlStr + "?ip=" + content + "&key=" + key);
            ...
            reader.close();
            if (JSONObject.parseObject(buffer.toString()).get("status") != null) {
                if (JSONObject.parseObject(buffer.toString()).get("status").toString().equals("0")) {
                    JSONObject jSONObject = JSONObject.parseObject(buffer.toString()).getJSONObject("result");
                    if (jSONObject != null && jSONObject.getJSONObject("ad_info") != null) {
                        return "" + jSONObject.getJSONObject("ad_info").get("city");
                    } else {
                        return "未知IP";
                    }
                } else {
                    return JSONObject.parseObject(buffer.toString()).get("message").toString();
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    // 过滤掉字符串中的html标签,还原所有的网页特殊符号实体
    public static String contentFilter2(String content) {
     ...
     * @return
     */
    public static String inVisible(String str) {
        String ragex = "[\u007f-\u009f]|\u00ad|[\u0483-\u0489]|[\u0559-\u055a]|\u058a|[\u0591-\u05bd]" +
                "|\u05bf|[\u05c1-\u05c2]|[\u05c4-\u05c7]|[\u0606-\u060a]|[\u063b-\u063f]|\u0674" +
                "|[\u06e5-\u06e6]|\u070f|[\u076e-\u077f]|\u0a51|\u0a75|\u0b44|[\u0b62-\u0b63]" +
                "|[\u0c62-\u0c63]|[\u0ce2-\u0ce3]|[\u0d62-\u0d63]|\u135f|[\u200b-\u200f]" +
                "|[\u2028-\u202e]|\u2044|\u2071|[\uf701-\uf70e]|[\uf710-\uf71a]|\ufb1e|[\ufc5e-\ufc62]" +
